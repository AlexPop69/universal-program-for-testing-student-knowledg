...
type
  ...
  private
    procedure Of1po;  //объекты для вопроса с одним правильным вариантом ответа
    procedure Of2po;  //объекты для вопроса с двумя правильными вариантами ответа
    procedure OfVO;   //объекты для вопроса с вводом ответа
    procedure OfVS;   //объекты для вопроса на соответсвия
    procedure Vvo;    //вопрос с вводом ответа
    procedure V1VO;   //вопрос с одним правильным вариантом ответа
    procedure V2VO;   //вопрос с двумя правильными вариантами ответа
    procedure Vso;    //вопрос на соответсвия
    procedure kon;    //конец теста
    procedure ShowTime;
  public
    { Public declarations }
  end;
...
NTE:='T'+IntToStr(s1+1);
  ...
  IF Ok.caption='Начать' then
    begin
      IF d=s then //выбор темы
        begin
          Nd:='D'+inttostr(s+1);
        end;
    end;
...
k:=StrToInt(str.Lines[0]); //количество вопросов
for i:= 1 to k Do
begin
otv.Lines.Add(' ');
end;
...
begin
  IF StrToInt(Vopros.Lines[15])=1 then //правильный ответ 1
    begin
      IF RB1.Checked  then
        begin
          n:=n+1;
  ...
  if strtoint(vopros.Lines[15])=2 then //правильный ответ 2
    begin
      if RB2.Checked  then
        begin
          n:=n+1;
  ...
  if strtoint(vopros.Lines[15])=3 then //правильный ответ 3
    begin
      if RB3.Checked  then
        begin
          n:=n+1;
  ...
  if strtoint(vopros.Lines[15])=4 then //правильный ответ 4
    begin
      if RB4.Checked  then
        begin
          n:=n+1;
...
begin
    ...
    n:=n+1;
    label2.Visible:=true;
    vopros.Lines.LoadFromFile(ExtractFilePath(Application.ExeName)+'\Disc\'+q+'\'+w+'\1.txt'); //загрузка первого вопроса
    label3.Caption:=vopros.Lines[0] + vopros.Lines[1]+vopros.Lines[2];
     label2.Caption:='Вопрос №' + inttostr(n)+' из '+IntToStr(k);  //вывод номера вопроса
//объекты для вопроса
...
ELSE IF n<>0 then
  begin
 label2.Caption:='Вопрос №' + IntToStr(n+1)+' из '+IntToStr(k);
